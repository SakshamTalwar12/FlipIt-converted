...
function Easy() {
  ...
  // Game state
  const [triesLeft, setTriesLeft] = useState(4); // Easy mode has 4 tries
  ...
  useEffect(() => {
    if (matchedPairs === 3) { // Easy mode has 3 pairs
      handleWin();
    }
  }, [matchedPairs]);
  ...
  return (
    <>
      ...
      <div className="difficulty-container">Difficulty: <span id="easy">Easy</span></div>
      
      <div className="back-btn" onClick={handleBackClick}>↖</div>
      
      <div className={`container1 ${gameOver || gameWon ? 'fade-out' : ''}`}>
        {cards.map(card => (
          <div 
            key={card.id} 
